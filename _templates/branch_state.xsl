...
<xsl:if test="*">
		...
		<div class="branch_details">
			...
			<div class="diagram">
				...
				<div class="devs" style="margin-bottom:10px;">
					<xsl:for-each select="data/online_dashs[*]"><ds title="{title}"></ds></xsl:for-each>
					<xsl:for-each select="data/offline_dashs[*]"><ds title="{title}" class="offline"></ds></xsl:for-each>
					&#160;
					<xsl:for-each select="data/online_kiosks[*]"><ko title="{title}"></ko></xsl:for-each>
					<xsl:for-each select="data/offline_kiosks[*]"><ko title="{title}" class="offline"></ko></xsl:for-each>
				</div>
	...
	<xsl:template match="/root/data">
            ...
            <xsl:for-each select="$settings/lane_list[id=$serviceId]">
            <xsl:sort select="title"/>
