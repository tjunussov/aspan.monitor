...
<xsl:if test="*">
         <ticket id="{id}">
            ...
            <pt><!--xsl:attribute name="time">
                    <xsl:call-template name="formatTime"><xsl:with-param name="value" select="t_st"/></xsl:call-template>
                </xsl:attribute>
                <xsl:attribute name="value">
	                <xsl:call-template name="toNum"><xsl:with-param name="value" select="t_st"/></xsl:call-template>
                </xsl:attribute-->
                <xsl:choose>
                    <xsl:when test="t_st!=''">
                        <xsl:call-template name="diffTime">
            ...
            <lane>
            	<xsl:call-template name="laneName">
                    <xsl:with-param name="branchId" select="lane_id"/>
	...
	<xsl:template match="/root/data">
            ...
            <tr>
                ...
                <th style="text-align:center; border:1px solid #444;">ВЫЗОВ</th>
            ...
            <xsl:for-each select="$details/ticket[status/@code='waiting'][position() &lt; 5]">
            <xsl:sort select="ot" order="descending"/>
            <xsl:sort select="no" order="descending"/>
             <tr title="{@id}">
                <td><t><xsl:value-of select="no"/></t>
            ...
            <xsl:for-each select="online_operators[unit_title]">
            <xsl:sort select="unit_title"/>
            <xsl:variable name="unit" select="."/>
	        <xsl:variable name="tickets" select="$details/ticket[win=$unit/unit_title]"/>
	        <xsl:variable name="tickets_work" select="$tickets[status/@code='pending' or status/@code='serving']"/>
	        
	        	<!-- EXIST SERVING PENDING -->
		        <tr title="{$tickets_work/status/@code}-{$tickets_work/@id}">	
		            <td>
		            	<t><xsl:value-of select="$tickets_work/no"/><xsl:if test="no/@is_booked"><span style="color:#FFF;">B</span></xsl:if></t>
		            	<xsl:choose>
		                	<xsl:when test="$tickets_work/no">
			                	<xsl:choose>
					                <xsl:when test="$tickets_work/no/@is_transfered">
						                <xsl:choose>
						                	<xsl:when test="$tickets_work/status='В'"><span class="blink">←</span></xsl:when>
						                	<xsl:otherwise>←</xsl:otherwise>
						                </xsl:choose>
					                </xsl:when>
					                <xsl:otherwise>
		            	...
		            	<w><xsl:value-of select="unit_title"/></w> <xsl:value-of select="username"/>
		            </td>
		           	<td><xsl:call-template name="formatTimeNum"><xsl:with-param name="value" select="$tickets_work/ot"/></xsl:call-template></td>
	                <td style="text-align:right;">
	                	<xsl:choose>
		                	<xsl:when test="$tickets_work">
		                		<xsl:attribute name="class"><xsl:if test="number($tickets_work/wt) &gt;= 15">max </xsl:if></xsl:attribute>
	                			<xsl:value-of select="format-number(number($tickets_work/wt),'0 МИН')"/>
		                	</xsl:when>
		                	<xsl:otherwise>
	                ...
	                <td style="text-align:right;">
	                	<xsl:attribute name="class"><xsl:if test="number($tickets_work/pt) &gt; 2">max </xsl:if><xsl:if test="$tickets_work/status/@code='pending'">  cur </xsl:if></xsl:attribute>
	                	<c><xsl:value-of select="format-number(number($tickets_work/pt),'0 МИН')"/></c></td>
	                <td style="text-align:right;">
	                	<xsl:attribute name="class"><xsl:if test="number($tickets_work/st) &gt;= 10">max </xsl:if><xsl:if test="$tickets_work/status/@code='serving'"> cur</xsl:if></xsl:attribute>
	                	<c><xsl:value-of select="format-number(number($tickets_work/st),'0 МИН')"/></c>
		            </td>
	                <td><xsl:value-of select="$tickets_work/lane"/></td>
		        </tr>
		        ...
		        <!-- DONE -->
		        <xsl:for-each select="$tickets[(status/@code='served' or status/@code='nocome-o' or status/@code='redirect')]">
            ...
            <xsl:for-each select="$details/ticket[status/@frozen]">
             <tr title="Заморожен - {@id}" class="frozen">
                <td><t><xsl:value-of select="no"/></t>
                	<xsl:if test="no/@is_booked"><span style="color:#FFF;">B</span></xsl:if>
	                <xsl:choose>
		                <xsl:when test="no/@is_transfered">
	                ...
	                <w><xsl:value-of select="win"/></w><span style="color:#777;">ОТЛОЖЕН...</span></td>
                <td><xsl:call-template name="formatTimeNum"><xsl:with-param name="value" select="ot"/></xsl:call-template></td>
                <td style="text-align:right;">
                	<xsl:if test="number(wt) &gt;= 15"><xsl:attribute name="class">max</xsl:attribute></xsl:if>
                	<xsl:value-of select="format-number(number(wt),'0 МИН')"/></td>
